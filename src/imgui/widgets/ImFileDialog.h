...
namespace ImGui {
    ...
    public:
        ...
        bool
        Open(const std::string &key, const std::string &title, const std::string &filter, bool isMultiselect = false,
             const std::string &startingDir = "");
        ...
        const std::filesystem::path &GetResult();

        const std::vector<std::filesystem::path> &GetResults();
        ...
        const std::vector<std::string> &GetFavorites();
...
#define FDLG ImGui::FileDialog::Instance()
