...
namespace ImGui {
    ...
    BaseKnob(const char *label, ImGuiDataType data_type, DataType *p_value, DataType v_min, DataType v_max, float speed,
        ...
        switch (variant) {
            ...
            case ImGuiKnobVariant_Space: {
                ...
                if (knob.t > 0.01) {
                    knob.draw_arc(0.4, 0.15, knob.angle_min - 1.0, knob.angle - 1.0, detail::GetPrimaryColorSet(), 16,
                                  2);
                    knob.draw_arc(0.6, 0.15, knob.angle_min + 1.0, knob.angle + 1.0, detail::GetPrimaryColorSet(), 16,
                                  2);
                    knob.draw_arc(0.8, 0.15, knob.angle_min + 3.0, knob.angle + 3.0, detail::GetPrimaryColorSet(), 16,
                                  2);
                }
                break;
            }
    ...
    color_set::color_set(ImColor base, ImColor hovered, ImColor active) : base(base), hovered(hovered),
                                                                          active(active) {}

    color_set::color_set(ImColor color) {
        base = color;
        hovered = color;
        active = color;
    }
